...
<style>
.headship{width:100%;margin-bottom:20px; padding:15px; border:1px solid #ddd;}
.headship table{width:100%;}
.headship table select{border: 1px solid #ddd;padding: 5px 7px;margin: 3px 4px; width:100%}
.headship table input{border: 1px solid #ddd;padding: 5px 7px;margin: 3px 4px; width:100%}
.headship table input:read-only{background: #eee;}
.headship .list_none{ padding: 49px 0;color: #999;text-align: center;}

.left{text-align:left;padding-left:5px}
.cen{text-align:center}
.ship_up_btn{background: #414350; color: #fff; width: 70px; height: 30px; border: 0;}
</style>
...
<div class="headship">
    <form id="WRK">
        <table>
            ...
            <tr>
                <td>
                    <?php if(empty($List[0]->TRS_REMARK)){ ?>
                        <select name="trgb" id="biz">
                            <option value="" <?= ($List[0]->TRS_REMARK)?'disabled':''; ?>>선택</option>
                            <option value="현물입고">현물입고</option>
                            <option value="선입고">선입고</option>
                            <option value="누락/결품">누락/결품</option>
                        </select>
                    <?php }else{ ?>
            ...
            <tr><td class="cen"><button type="button" class="ship_up_btn"><?= ($List[0]->TRS_REMARK)?'완료':'저장'; ?></button></td></tr>
            <?php } }else{ ?>
...
$(document).off("change",".headship table tr:eq(9) td input");
$(document).on("change",".headship table tr:eq(9) td input",function(){
    if( $(this).val() >  $(this).data('max')){ $(this).val( $(this).data('max')); }
    if( $(this).val() < 0){ $(this).val(0); }
    $('.headship table tr:eq(11) td input').val(($(this).val() * $(this).data('weight')).toFixed(3))
})
// tr클릭시 체크 설정
$(document).off("click",".ship_up_btn");
$(document).on("click",".ship_up_btn",function(){
    if($("select[name=trgb]").val() == '' || $("input[name='date']").val() == '' || $("input[name='weight']").val() == '0.000'){
        alert("필수사항을 입력해주세요")
        return false;
    }    
    var yn = 'N'
    if($(".headship table").find('.ship_up_btn').text() == '완료'){
        yn = 'Y'
    }

    shipUp = new FormData($("#WRK")[0]);
    shipUp.append('yn', yn);
	$.ajax({
		url: "<?= base_url('/TSK/ship_up/') ?>",
		type: "post",
		data : shipUp,
		dataType: "html",
		cache  : false,
