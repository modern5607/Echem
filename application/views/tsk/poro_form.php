				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td class="right"><?= NUMBER_FORMAT($WEIGHT); ?></td>
							<td class="right"></td>
						</tr>
						<?php } ?>
					</tbody>
...
$(document).on("click",".register_form table tr",function(){
	...
	var weight = $("input[name='QW']").data("weight") * 1 ;

	// 표시할 변경값
	var minusQ = checked.find('td:eq(3)').text().replace(/[^\d]+/g, '')*1-qty;
	if(minusQ < 0){minusQ = 0;}
	var minusQty = Math.round(minusQ).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
	var plusQty = Math.round($(this).find('td:eq(3)').text().replace(/[^\d]+/g, '')*1+qty).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");

	var minusW = checked.find('td:eq(4)').text().replace(/[^\d]+/g, '')*1-weight;
	if(minusW < 0){minusW = 0;}
	var minusWeight = Math.round(minusW).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
