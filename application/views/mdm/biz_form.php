								...
								<select name="CUST_TYPE" id="CUST_TYPE" class="form_input">
									...
									<?php foreach ($BIZGB as $i => $row) {
										...
										if (!empty($info) && isset($info)) {
											$selected = ($row->D_NAME == $info->CUST_TYPE) ? 'selected' : '';
										} ?>
										<option value="<?= $row->D_NAME ?>" <?= $selected ?>><?= $row->D_NAME ?></option>
									<?php } ?>
								</select>
							</td>
						</tr>
...
<script>
	...
	// $(document).on("keyup", "#CUST_NM", function() {
	// 	bizchk = 0;
	// });
